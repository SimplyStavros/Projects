...
def profile():
    ...
    if form.validate_on_submit():
        ...
        valid = User.authenticate(user.username, form.password.data)

        if valid:
            user.username = form.username.data or user.username
            user.email = form.username.data or user.email
...
def homepage():
    ...
    if g.user:
        ...
        user_ids_to_query = followed_user_ids + [g.user.id]
        messages = (Message
                    .query
                    .filter(Message.user_id.in_(user_ids_to_query))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
